...
@interface QRCodeManager ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIGestureRecognizerDelegate, QRCodeViewDelegate>
...
- (instancetype)initWithPreviewView:(QRCodeView *)previewView completion:(void (^)(void))completion {
    ...
    if (self) {
        ...
        if ([previewView isKindOfClass:[QRCodeView class]]) {
            _previewView = (QRCodeView *)previewView;
...
- (void)stopScanning {
    ...
    [QRCodeManager switchTorch:NO];
    [QRCodeManager resetZoomFactor];
}

- (void)presentPhotoLibraryWithRooter:(UIViewController *)rooter callback:(void (^)(NSString * _Nonnull))callback {
    _callback = callback;
...
- (void)codeScanningView:(QRCodeView *)scanningView didClickedTorchSwitch:(UIButton *)switchButton {
    ...
    [QRCodeManager switchTorch:switchButton.selected];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size {
    ...
    UIImage *codeImage = [QRCodeManager scaleImage:filter.outputImage toSize:size];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size logo:(nonnull UIImage *)logo {
    ...
    UIImage *codeImage = [QRCodeManager generateQRCode:code size:size];
    codeImage = [QRCodeManager combinateCodeImage:codeImage andLogo:logo];
